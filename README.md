...
*   **Положення про Студентську раду Студмістечка (СР СМ):** Документ, що регламентує діяльність СР СМ - виконавчого органу студентського самоврядування в побутовій сфері. (Знаходиться в директорії `Положення_про_СР_СМ/`)
...
### Перехідні документи
*   **Положення про Тимчасовий перехідний комітет (ТПК):** Документ, що регламентує діяльність ТПК - тимчасового виконавчого органу, який діє під керівництвом КСУ до формування постійних органів студентського самоврядування. (Знаходиться в директорії `Положення_про_ТПК/`)

### Виборчі документи
*   **Положення про Центральну виборчу комісію студентів (ЦВКс):** Документ, що регламентує діяльність ЦВКс - органу, відповідального за організацію та проведення виборів в системі ОСС. (Знаходиться в директорії `Положення_про_ЦВКс/`)
*   **Тимчасовий виборчий регламент:** Документ, що встановлює процедури та правила проведення виборів до органів студентського самоврядування. (Знаходиться в директорії `Тимчасовий_виборчий_регламент/`)

### Регламентні документи
*   **Регламент Конференції студентів Університету (КСУ):** Документ, що визначає порядок скликання, проведення та прийняття рішень на Конференції студентів. (Знаходиться в директорії `Регламент_КСУ/`)

## Структура

Кожен документ складається з головного `.tex` файлу, який підключає окремі файли для кожного розділу (`Розділ_*.tex`). Наприклад:
- `Положення_про_ОСС.tex`
- `Положення_про_СР_КАІ.tex`
- `Положення_про_СР_СМ.tex`
- `Положення_про_СУД.tex`
- `Положення_про_ТПК.tex`
- `Положення_про_ЦВКс.tex`
- `Регламент_КСУ.tex`
- `Тимчасовий_виборчий_регламент.tex`
...
*   **`make all`** або просто **`make`**: Скомпілює усі документи. Результуючі PDF файли будуть розміщені в директорії `dist/`:
    - `Polozhennia-pro-OSS.pdf`
    - `Polozhennia-pro-SR-KAI.pdf`
    - `Polozhennia-pro-SR-SM.pdf`
    - `Polozhennia-pro-SUD.pdf`
    - `Polozhennia-pro-TPK.pdf`
    - `Polozhennia-pro-CVKS.pdf`
    - `Reglament-KSU.pdf`
    - `Tymchasovyi-vyborchyi-reglament.pdf`
...
*   **При створенні та push тегу, що починається з `v` (наприклад, `v1.0`, `v1.1.2`):** Документи компілюються, і робочий процес намагається оновити існуючий реліз на GitHub з цим тегом (або створити новий, якщо він не існує). Всі скомпільовані PDF файли прикріплюються як активи до цього релізу.
